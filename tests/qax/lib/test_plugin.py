...
class MyOtherPlugin(QaxCheckToolPlugin):
    supported_file_types = [
        QaxFileType(name="shp file", extension="shp", group="Shapefiles")
    ]
...
class TestQaxPlugins(unittest.TestCase):
    check_tool_config = QaxConfigCheckTool.from_dict(
        {"name": "test check tool", "pluginClass": "tests.qax.lib.test_plugin.MyPlugin"}
    )
    ...
    def test_load_plugin(self):
        ...
        check_tool_plugin = plugins._load_plugin(
            TestQaxPlugins.check_tool_profile, TestQaxPlugins.check_tool_config
        )
    ...
    def test_file_group_from_dict(self):
        ...
        self.assertEqual(len(file_group_dict["fileTypes"]), len(file_group.file_types))
    ...
    def test_file_group_merge(self):
        ...
        raw_prod = next((p for p in merged_prods if p.name == "Raw Products"), None)
        ...
        dtm_prod = next((p for p in merged_prods if p.name == "Survey DTMs"), None)
