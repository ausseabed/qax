...
class TestQaxConfig(unittest.TestCase):
        ...
        "specifications": [
            {
                ...
                "checks": [
                    {"checkId": "cid1", "parameters": [{"name": "p1", "value": 1}]},
                    {"checkId": "cid2", "parameters": [{"name": "p2", "value": 2}]},
                ],
            },
            {
                "name": "s2",
                "checks": [
                    {"checkId": "cid1", "parameters": [{"name": "p3", "value": 3}]},
                    {
                        "checkId": "cid2",
                        "parameters": [
                            {"name": "p4", "value": 4},
    ...
    def test_qax_config_load(self):
        ...
        self.assertTrue(any(p.name == "Flier Finder" for p in profile.check_tools))
        self.assertTrue(any(p.name == "Coverage check" for p in profile.check_tools))
