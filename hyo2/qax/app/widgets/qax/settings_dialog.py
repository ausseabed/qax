...
class SettingsDialog(QDialog):
    ...
    def _load_data_from_config(self) -> None:
        ...
        log_qax_val = (
            gui_settings_const.logging_qax_default
            if log_qax_val is None
            else log_qax_val
        )
        log_qt_val = (
            gui_settings_const.logging_qt_default if log_qt_val is None else log_qt_val
        )
        log_other_val = (
            gui_settings_const.logging_other_default
            if log_other_val is None
            else log_other_val
        )
        ...
        log_other_index = [y[0] for y in gui_settings_const.LOG_LEVELS].index(
            log_other_val
        )
    ...
    def _add_gridprocessing(self) -> None:
        ...
        gridprocessing_groupbox.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        processingtile_label_0 = QLabel("Processing tile dimensions")
        ...
        processingtile_label_1.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Minimum)
        ...
        self.processingtile_x.textChanged.connect(self._on_processingtile_x_changed)
        self.processingtile_y.textChanged.connect(self._on_processingtile_y_changed)
    ...
    def _on_processingtile_x_changed(self, x):
        GuiSettings.settings().setValue(
            gui_settings_const.gridprocessing_tile_x, self.__sanitise_tile_size(x)
        )

    def _on_processingtile_y_changed(self, y):
        GuiSettings.settings().setValue(
            gui_settings_const.gridprocessing_tile_y, self.__sanitise_tile_size(y)
        )
    ...
    def _add_logging(self) -> None:
        ...
        logging_groupbox.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        ...
        label_1.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Minimum)
        ...
        label_2.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Minimum)
        ...
        lbl = QLabel("QAX logging:")
        lbl.setFixedWidth(label_width)
        layout_qax_logging.addWidget(lbl)
        ...
        lbl = QLabel("QT logging:")
        lbl.setFixedWidth(label_width)
        layout_qt_logging.addWidget(lbl)
        ...
        lbl = QLabel("Other logging:")
        lbl.setFixedWidth(label_width)
        layout_other_logging.addWidget(lbl)
    ...
    def _on_logging_changed(self):
        GuiSettings.settings().setValue(
            gui_settings_const.logging_qax, self.cb_qax_logging.currentText()
        )
        GuiSettings.settings().setValue(
            gui_settings_const.logging_qt, self.cb_qt_logging.currentText()
        )
        GuiSettings.settings().setValue(
            gui_settings_const.logging_other, self.cb_other_logging.currentText()
        )

        log_qax_int = [
            item[1]
            for item in gui_settings_const.LOG_LEVELS
            if item[0] == self.cb_qax_logging.currentText()
        ][0]
        log_qt_int = [
            item[1]
            for item in gui_settings_const.LOG_LEVELS
            if item[0] == self.cb_qt_logging.currentText()
        ][0]
        log_other_int = [
            item[1]
            for item in gui_settings_const.LOG_LEVELS
            if item[0] == self.cb_other_logging.currentText()
        ][0]
