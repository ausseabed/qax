...
class ResultTab(QtWidgets.QWidget):
    ...
    def __init__(self, prj: QAXProject):
        ...
        for idx, view_name in enumerate(self.view_names):
            ...
            viewRadioButton.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Minimum)
        ...
        self.set_data_level.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Minimum)
    ...
    def _add_summary_view(self):
        ...
        self.summary_sort_proxy_model.setSourceModel(self.summary_table_model)
    ...
    def _on_clicked_summary(self, selected_index):
        ...
        source_index = self.summary_sort_proxy_model.mapToSource(selected_index)
    ...
    def _update_json_view(self):
        ...
        if (
            ("qa" in qa_json_dict)
            and (qa_json_dict["qa"] is not None)
            and (self.qa_group in qa_json_dict["qa"])
        ):
            try:
    ...
    def _add_score_board_view(self):
        ...
        self.scoreboard_sort_proxy_model.setSourceModel(self.scoreboard_table_model)
    ...
    def __splitterMoved(self, pos: int, index: int) -> None:
        ...
        GuiSettings.settings().setValue(
            gui_settings_const.result_scoreboard_splitter_v_loc, " ".join(sizes)
        )
    ...
    def _update_score_board_view(self):
        ...
        if GuiSettings.settings().contains(
            gui_settings_const.result_scoreboard_splitter_v_loc
        ):
            loc = GuiSettings.settings().value(
                gui_settings_const.result_scoreboard_splitter_v_loc, type=str
            )
    ...
    def _on_clicked_scoreboard(self, selected_index):
        ...
        source_index = self.scoreboard_sort_proxy_model.mapToSource(selected_index)
        selected_row = source_index.row()
        ...
        self.scoreboard_details.set_selected_check(self.scoreboard_selected_check)
    ...
    def _display_json(self):
        ...
        for row in range(self.set_data_level.count()):
            ...
            data_level_available = getattr(self.qa_json.qa, data_level_name) is not None
    ...
    def _on_set_view(self):
        ...
        GuiSettings.settings().setValue(
            gui_settings_const.result_selected_tab, self.cur_view
        )
