...
def setup_logging() -> None:
    ...
    log_qax_val = (
        gui_settings_const.logging_qax_default if log_qax_val is None else log_qax_val
    )
    log_qt_val = (
    ...
    log_qax_int = [
        item[1] for item in gui_settings_const.LOG_LEVELS if item[0] == log_qax_val
    ][0]
    log_qt_int = [
        item[1] for item in gui_settings_const.LOG_LEVELS if item[0] == log_qt_val
    ][0]
    log_other_int = [
        item[1] for item in gui_settings_const.LOG_LEVELS if item[0] == log_other_val
    ][0]

    set_logging(
        default_logging=log_other_int, qax_logging=log_qax_int, qt_logging=log_qt_int
    )

    logger = logging.getLogger(__name__)
    logger.debug(
        f"log levels: QAX-{log_qax_int}, default-{log_other_int}, QT-{log_qt_int}"
    )
